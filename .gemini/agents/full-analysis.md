...
For each stage, use the `invoke_agent` tool to invoke the appropriate agent:

#### Stage 1: Data Cleaning

```
Invoke data-cleaner via `invoke_agent` tool:
...
```
Invoke data-analyzer via `invoke_agent` tool:
...
```
Invoke data-visualizer via `invoke_agent` tool:
...
```
Invoke report-writer via `invoke_agent` tool:
...
```
Invoke exec-summarizer via `invoke_agent` tool:
- agent_name: "exec-summarizer"
- prompt: |
